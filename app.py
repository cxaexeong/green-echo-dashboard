...
if search_name:
    ...
    if find_target.empty:
        ...
        cluster_name_map = {0: "유사기관군 A", 1: "유사기관군 B"}
        ...
        if not target_df.empty:
            ...
            txt_ai_criteria = (
                "유사기관군 A/B는 최근 3년 구매 실적이 비슷한 기관들을 묶어 구분한 유형입니다.<br><br>구분 기준은 다음 2가지입니다.<br><br>"
        ...
        def make_current_dashboard():
            ...
            if not target_df.empty:
                ...
                fig.add_trace(
                    go.Table(
                        ...
                        header=dict(values=["", "", ""], height=0, line_color=trans_col),
                        cells=dict(
                            values=[[get_pattern_html("유사기관군 A"), ""], [get_pattern_html("유사기관군 B"), ""]],
                            fill_color=[["white", trans_col]] * 2,
                            line_color=[[tbl_line, trans_col]] * 2,
            ...
            fig.update_layout(
                title=dict(text=f"<b>{CURRENT_YEAR}년 공공기관 녹색구매 위치찾기{title_suffix}</b>", x=0.5, xanchor="center", font=dict(size=26)),
            ...
            fig.add_annotation(xref="x1", yref="y1", x=np.log10(horizontal_line_x1), y=ai_ratio_line, text=ratio_label, showarrow=False, font=dict(size=14, color="black"), xanchor="right", yanchor="bottom", yshift=5)
            fig.add_annotation(xref="x1", yref="y1", x=np.log10(ai_amount_line), y=vertical_line_y1, text=amount_label, textangle=-90, showarrow=False, font=dict(size=14, color="black"), xanchor="center", xshift=15, yanchor="top")
            ...
            fig.add_annotation(xref="x domain", yref="paper", x=0.5, y=0.49, text=("<span style='color:#CDB4DB'>●</span> 유사기관군 A&nbsp;&nbsp;&nbsp;<span style='color:#FDFD96'>●</span> 유사기관군 B&nbsp;&nbsp;&nbsp;<span style='color:#000000'>★</span> 검색 기관"), showarrow=False, xanchor="center", yanchor="middle", font=dict(size=12), bgcolor="white", bordercolor="#d9d9d9", borderwidth=1, borderpad=6)
            if not target_df.empty:
